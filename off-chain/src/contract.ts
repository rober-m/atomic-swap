...
// datum that states who must be paid (A) and the `price` A is asking for. Any
// Person B can then ACCEPT the swap by spending that UTxO, provided the
// transaction contains exactly one output that pays A at least the `price` and
// carries the SWAP UTXO'S OWN OUTPUT REFERENCE as its datum. That output
// reference is globally unique, so the payment is bound to exactly this swap and
// one payment can never settle two swaps (double satisfaction). B keeps the
// locked assets; A receives the price — atomically, in one transaction. A can
// also CANCEL an unaccepted swap and reclaim the locked assets by signing.
...
export class SwapContract {
    ...
    /** Build the inline datum (owner pkh, price) attached to a locked UTxO. */
    private buildDatum(owner: string, price: Asset[]) {
        ...
        return mConStr0([owner, pricedAssets]);
    }
    ...
    readTerms(swapUtxo: UTxO): SwapTerms {
        ...
        // The datum is Constr0 [ owner: bytes, price: [Constr0 [policy, name, qty]] ].
     ...
     * asking `price` in return. Called by Person A. Returns the UNSIGNED
     * transaction hex.
     */
    lock = async (lockedAssets: Asset[], price: Asset[]): Promise<string> => {
        ...
        const tx = await this.newTxBuilder()
            ...
            .txOutInlineDatumValue(this.buildDatum(owner, price))
     ...
     * owner the asked `price` in a single output marked with the swap UTxO's own
     * output reference. Called by Person B. Returns the UNSIGNED transaction hex.
     */
    accept = async (swapUtxo: UTxO): Promise<string> => {
        const { utxos, walletAddress, collateral } = await this.getWalletInfoForTx();
        const { owner, price } = this.readTerms(swapUtxo);
        ...
        // The payment leg is bound to this exact swap by marking it with the swap
        // UTxO's own output reference — which is what the validator checks.
        const mark = mOutputReference(swapUtxo.input.txHash, swapUtxo.input.outputIndex);
        ...
        const tx = await this.newTxBuilder()
            ...
            // The single payment leg: pays the owner the price, marked with this
            // swap UTxO's output reference so it binds to exactly this swap.
            .txOut(ownerAddress, ensureLovelace(price))
            .txOutInlineDatumValue(mark)
            .changeAddress(walletAddress)
            .txInCollateral(
                collateral.input.txHash,
                collateral.input.outputIndex,
